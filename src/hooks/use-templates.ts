...
export const useCreateTemplate = () => {
  ...
  return useMutation({
    mutationFn: async (templateData: CreateTemplateData): Promise<DocumentTemplate> => {
...
export const useUpdateTemplate = () => {
  ...
  return useMutation({
    mutationFn: async ({ id, ...updateData }: UpdateTemplateData): Promise<DocumentTemplate> => {
      const { data, error } = await supabase
        .from("document_templates")
        .update({
          ...updateData,
...
export const useDuplicateTemplate = () => {
  ...
  return useMutation({
    mutationFn: async (templateId: string): Promise<DocumentTemplate> => {
      ...
      if (fetchError) {
        throw new Error(`Erro ao buscar template original: ${fetchError.message}`);
      }
