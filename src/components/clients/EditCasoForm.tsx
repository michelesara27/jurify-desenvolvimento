...
export function EditCasoForm({ open, onOpenChange, caso, onCasoAtualizado }: EditCasoFormProps) {
  ...
  useEffect(() => {
    if (caso && open) {
      form.reset({
        ...
        start_date: caso.start_date ? new Date(caso.start_date).toISOString().split('T')[0] : "",
        end_date: caso.end_date ? new Date(caso.end_date).toISOString().split('T')[0] : "",
      });
    }
  }, [caso, open, form]);
