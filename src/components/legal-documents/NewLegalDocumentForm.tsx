...
const legalDocumentSchema = z.object({
  ...
  document_type: z.enum(["peticion", "contract", "appeal", "motion", "brief", "memorandum", "other"], {
    required_error: "Selecione um tipo de documento",
  }),
  status: z.enum(["draft", "review", "approved", "filed", "archived"]).default("draft"),
  ...
  pages_count: z.number().min(1, "Documento deve ter pelo menos 1 página").optional(),
...
export function NewLegalDocumentForm({ onSuccess, onCancel }: NewLegalDocumentFormProps) {
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input 
                        placeholder="Nome do réu da ação"
                        {...field}
                      />
                    </FormControl>
                    <FormMessage />
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <div className="flex justify-between text-sm text-muted-foreground">
                    <span>Palavras: {estimateWordCount(form.watch('content') || '')}</span>
                    <span>Páginas estimadas: {estimatePageCount(form.watch('content') || '')}</span>
                  </div>
                  <FormMessage />
                </FormItem>
              )}
